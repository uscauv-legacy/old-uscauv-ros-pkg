...
void Serial::connect()
{
        ...
        printf("INFO: Looking Device Name [%s]",itsDevName.c_str());
        if(itsDevName != "")
        {
                printf("INFO: Opening %s", itsDevName.c_str());
        ...
        write(cmd,1);
                ...
                if(itsCmdDevName == "")
                {
                        printf("FATAL ERROR: Could Not Find Serial Device Matching Descriptor (%s)", itsDeviceDescription.c_str());
                }
        } else {
                printf("INFO: Opening from cmd line %s", itsCmdDevName.c_str());
...
int Serial::openPort(std::string DeviceName)
{
        ...
        printf("DEBUG: Opening port %s", DeviceName.c_str());
