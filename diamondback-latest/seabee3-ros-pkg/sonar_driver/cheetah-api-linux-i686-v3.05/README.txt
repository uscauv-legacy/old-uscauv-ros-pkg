...
Introduction
------------
This software is used to interface with the Cheetah SPI Adapter.
It provides APIs in a number of languages for maximum development
flexibility.
...
plugging in any devices or running software.  Refer to the Cheetah
datasheet for more details on the Windows USB driver.

Under Linux, the libusb library must be installed since the Cheetah
...
please refer to the Cheetah datasheet and the example source code available
...
2) Add cheetah.c and cheetah.h to the project
3) Place cheetah.dll (or cheetah.so for Linux/Darwin) in the PATH
4) Place #include "cheetah.h" in any module that uses the API
...
1) Copy cheetah_py.py to a new folder
2) Copy cheetah.dll (or cheetah.so for Linux/Darwin) to the same folder
...
There are two main difference between the Cheetah API documented in the
datasheet and the Cheetah Python API.

1) The "array" type is used for passing data to and from the
Cheetah Python API.  See the Python documentation for more
information on this type.

2) Cheetah Python API functions can return multiple arguments on
...
The calling conventions for each Cheetah Python API function is
documented in the comments of cheetah_py.py.


C# bindings
-----------
1) Create a new C/C++ project or open an existing project
2) Add cheetah.cs to the project
3) For Windows, place cheetah.dll in the PATH;
   For Linux rename cheetah.so to libcheetah.so and place in the
   LD_LIBRARY_PATH
...
------------------------
Copy cheetah.dll and cheetah_net.dll to your application development
environment.  The cheetah.dll contains the APIs themselves and the
cheetah_net.dll provides the .NET interface to the APIs.  For
detailed documentation of APIs refer to the datasheet and the comments
in the C# (cheetah.cs) binding.
...
Visual Basic 6
--------------
Copy cheetah.dll to your application development environment and
integrate the cheetah.bas interface layer into your application.
For detailed documentation of the APIs refer to the datasheet
and the comments in the cheetah.bas file.
