...
for i = 1:rows( sample_pts_u )
	# generate drag vectors for vertical and horizontal faces
	...
	# the drag vector along the y axis 
	drag_vecs(i,:) = [ -sample_vecs_u(i,1), 0 ]
	drag_vecs(i+rows(sample_pts_u),:) = [ 0, -sample_vecs_v(i,2) ]
end
...
drag_vecs = [drag_vecs; -drag_vecs ]

# transform from 3D x/y plane to 2D x/y plane
sample_vecs = sample_vecs * [ 0 1; -1 0 ]
sample_pts = sample_pts * [ 0 1; -1 0 ]
drag_vecs = drag_vecs * [ 0 1; -1 0 ]
bounds_graph = bounds_graph * [ 0 1; -1 0 ]
bounds_points = bounds_points * [ 0 1; -1 0 ]
...
for i = 1:rows( sample_vecs )
	...
	quiver( sample_pts(i,1), sample_pts(i,2), drag_vecs(i,1), drag_vecs(i,2), "1" )
end